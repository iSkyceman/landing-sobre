...
En cas de litige, les parties s&apos;efforcent de résoudre leur différend à l&apos;amiable. À défaut, le litige
...
const verifyDataPlusCode = async (code: string, clientId?: string) => {
  console.log('🔍 Vérification code Data+ Itech:', code);
  ...
  if (code.includes('DATAPLUS')) {
    console.log('✅ Code Data+ Itech valide (mode simulation)');
...
export default function OptionDataPlusItech({ prefillData: externalPrefillData }: { prefillData?: PrefillDataType }) {
  ...
  function buildContractHtml(): string {
    ...
    return `
      ...
      <h3>Abonnement Data+ ITECH <span style="font-size: 1.2em;">🚀</span></h3>
    ...
    if (printWindow) {
      ...
      printWindow.document.write(`
        ...
        <html lang="fr">
          <head>
            <meta charset="UTF-8" />
            <title>Contrat Data+ ITECH</title>
            <style>
              button.print-btn {
                margin-top: 20px;
                padding: 12px 20px;
